...
//	}
	...
	public String queryEmployee(){
		...
		List<HotelBean> hotels =	(List<HotelBean>)session.getAttribute("hotelsAll");
		...
		if(errorNo == 0){
			...
			for (EmployeeBean e : ems) {
				...
				for (HotelBean h : hotels) {
					if(h.getId()==e.getHotel_id()){
						e.setHotelStr(h.getName());
						break;
					}
				}
			...
			request.setAttribute("hotel_id_b", hotel_id);
