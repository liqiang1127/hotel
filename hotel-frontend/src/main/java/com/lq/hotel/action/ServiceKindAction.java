...
public class ServiceKindAction extends BaseAction implements ModelDriven<ServiceKindBean>{
	...
	public String queryHotelServiceKind() {
		...
		String json = MiddleCilent.sendHttpJsonRequest(FunctionConstant.queryHotelServiceKind, JSON.toJSONString(reqMap));
		resMap = JSON.parseObject(json);
		List<ServiceKindBean> serviceKinds = JSON.parseObject(resMap.get("items").toString(),
				new TypeReference<List<ServiceKindBean>>() {
				});
		request.setAttribute("serviceKinds", serviceKinds);
		return "list";
	}
	...
	public String editHotelServiceKind(){
		...
		reqMap.put("hotel_id", (int)session.getAttribute("hotel_id"));
		reqMap.put("serviceKind_id", roomKind_id);
		reqMap.put("op", op);
		 MiddleCilent.sendHttpJsonRequest(FunctionConstant.editHotelServiceKind, JSON.toJSONString(reqMap));
		return "list";
	}
	@Override
